...
- **Live CEF Ephemeral Profile Lifecycle**: When entering Private Mode, `DesktopApp` allocates an ephemeral profile, creates an isolated `is_ephemeral` tab backed by an in-memory `CefRequestContext`, ensures subsequent tabs opened in private mode inherit `is_ephemeral = true`, and purges all private tabs, secrets, and in-memory contexts upon exit. `SessionHistoryBridge` and `SessionPersistence` enforce that private visits and focus items never reach disk history or session snapshots. Tested via `openbrowser_private_profile_isolation_tests`.
...
- **Live Connection Endpoint Tracking & Telemetry Authenticity**: Endpoint identifiers (`conn:<host>:<port>`) are parsed from live CEF responses and registered in `ConnectionRegistry`, linking `NetworkEvent::connection_id` to observable connection diagnostics. In accordance with telemetry authenticity rules, TLS version, cipher suite, cert subject, and protocol negotiation are not fabricated from standard HTTP callbacks, remaining nullopt/unknown until deeper transport instrumentation is integrated.
- **Live User-Agent & Mitigation Enforcement with Platform Detection**: `CefTabClient` uses explicit platform detection (`CurrentPlatformName()`) to supply accurate OS tokens (`Windows`, `Linux`, `macOS`) to `UserAgentPolicyEngine` for standard mode, while respecting uniform anti-fingerprinting and site-scoped compatibility mitigations.
